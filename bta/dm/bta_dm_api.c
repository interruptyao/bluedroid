...
/*******************************************************************************
**
** Function         BTA_DmNPLSpecificCommand
**
** Description      This function sends the vendor specific command
**                  to the controller
**
...
**
*******************************************************************************/
tBTA_STATUS BTA_DmNPLSpecificCommand (UINT16 opcode, UINT8 sub_opcode,UINT8 param_len,
                                         UINT8 *p_param_buf,
                                         tBTA_VENDOR_CMPL_CBACK *p_cback)
{
	tHCI_VSC_NPL vsc;
	vsc.sub_opcode = sub_opcode;
	memset(&vsc.buf,0x0,sizeof(vsc.buf));
	memcpy(&vsc.buf,p_param_buf,param_len);
	return BTA_DmVendorSpecificCommand(opcode,param_len + sizeof(vsc.sub_opcode),(UINT8 *)&vsc,p_cback);
}
...
BTA_API extern void BTA_DMUpdateEir(void)
{
    APPL_TRACE_API("%s",__func__);
    tBTA_DM_UPDATE_EIR *pmsg = NULL;
    pmsg = GKI_getbuf(sizeof(tBTA_DM_UPDATE_EIR));
    if (pmsg != NULL)
    {
        pmsg->hdr.event = BTA_DM_UPDATE_EIR_REQ;
