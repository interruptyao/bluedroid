...
/*******************************************************************************
**
** Function         bta_dm_update_did_eir_req
**
...
**
*******************************************************************************/
void bta_dm_update_did_eir_req(tBTA_DM_MSG *p_data)
{
    APPL_TRACE_API("bta_dm_update_eir_req");
    bta_dm_set_eir(NULL);
}
...
static void bta_dm_set_eir (char *local_name)
{
    ...
    if (free_eir_length > 9)
    {
        ...
        if (BTA_DmGetLocalDiRecord(&DID, &p_handle) == BTM_SUCCESS)
        {
            ...
            APPL_TRACE_ERROR("DID.rec.vendor_id_source=%d,DID.rec.vendor=%d,DID.rec.product=%d,DID.rec.version=%d",DID.rec.vendor_id_source,DID.rec.vendor,DID.rec.product,DID.rec.version);
            free_eir_length -= (len +1);
        }
        else
            APPL_TRACE_ERROR("Failed to get Local DID record");
    }
...
//XXX ifdef?
    ...
    BTM_SetConnectability(BTM_CONNECTABLE,BTM_DEFAULT_CONN_WINDOW,BTM_DEFAULT_CONN_INTERVAL);
    BTM_SetDiscoverability(BTM_GENERAL_DISCOVERABLE, BTM_DEFAULT_DISC_WINDOW, BTM_DEFAULT_DISC_INTERVAL);
}
#endif
...
{
    APPL_TRACE_DEBUG("%s: Enter enable = %d", __FUNCTION__,p_data->enable);
