...
void bte_load_conf(const char *path) {
  ...
  enable_test_mode = config_get_bool(config, CONFIG_DEFAULT_SECTION, "EnableTestMode", false);
  unconditional_patch_upgrade = config_get_bool(config, CONFIG_DEFAULT_SECTION, "UnconditionalPatchUpgrade", false);
