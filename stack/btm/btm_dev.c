...
static  void link_key_del_cback(tBTM_VSC_CMPL *p_params)
{
	...
	switch(sub_opcode)
	{
		case HCI_VSC_HOGP_DELETE_DEVICE_INFO:
			BTIF_TRACE_DEBUG(" %s####link_key_del_cback delete link key	status:%d",__FUNCTION__,status);
			break;
		default:
			BTIF_TRACE_DEBUG(" %s####link_key_del_cback error status:%d",__FUNCTION__,status);
			break;
...
BOOLEAN BTM_SecDeleteDevice (BD_ADDR bd_addr)
{
	...
	//if(p_dev_rec->dev_class[1] == BTM_COD_MAJOR_PERIPHERAL)
	if(hid_known_hogp_device(bd_addr)) // TODO:wrap this function
	{

		for(unsigned int i = 0;i < sizeof(BD_ADDR);i++)
		{
			p.bd_addr[i] = bd_addr[5-i];
		}
		p.addr_type = p_dev_rec->ble.ble_addr_type;
		BTA_DmNPLSpecificCommand(HCI_VSC_MAIN_NPL,HCI_VSC_HOGP_DELETE_DEVICE_INFO ,sizeof(BD_ADDR) + 1, (UINT8 *)&p, link_key_del_cback);
        BTIF_TRACE_DEBUG(" %s HOGP delete link key",__FUNCTION__);
	}
	else
	{
        BTIF_TRACE_DEBUG(" %s It is not HOGP device",__FUNCTION__);
	}
