    ...
    if (!p_cb)
    {
        ...
        if (bta_hh_cb.p_cback != NULL)
        {
            ...
            case BTA_HH_API_GET_DSCP_EVT:
                APPL_TRACE_ERROR("event is 0x%x,handle %d:", event,p_data->hdr.layer_specific);
                cback_event = BTA_HH_CLOSE_EVT;
                cback_data.dev_status.status = BTA_HH_ERR_HDL;
...
#if BTA_HH_DEBUG
            APPL_TRACE_DEBUG("bta_hh_hdl_event:: event = %x handle = %d dev_cb[%d] ", p_msg->event ,p_msg->layer_specific, index);
#endif
