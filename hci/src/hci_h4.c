...
#define HCI_VSC_MAIN_NPL            0xFC36
#define HCI_VSC_FIRMWARE_UPGRADE    0x0C
#define HCI_VSC_HEADLESS_ENABLE     0x04
...
void hci_h4_send_msg(HC_BT_HDR *p_msg)
{
    ...
    *p = type;
    ...
    if (event == MSG_STACK_TO_HC_HCI_CMD)
    {
        ...
        if ((lay_spec == HCI_VSC_MAIN_NPL) && (*(p + 1) == HCI_VSC_FIRMWARE_UPGRADE)) {
            fw_upgrade_in_prog = TRUE;
            ALOGE("%s: Sending VSC to erase the firmware...", __func__);
        } else if ((lay_spec == HCI_VSC_MAIN_NPL) && (*(p + 1) == HCI_VSC_HEADLESS_ENABLE)) {
			fw_headless_mode_enable = 1;
		} else
