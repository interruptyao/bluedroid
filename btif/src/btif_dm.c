...
static void btif_dm_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_DM_BLE_LOCAL_IR_EVT:
			...
			BTM_WriteLocalIRK(ble_local_key_cb.id_keys.irk);
...
static  void link_key_write_cback(tBTM_VSC_CMPL *p_params)
{
	...
	switch(sub_opcode)
	{
		case HCI_VSC_HOGP_ADD_DEVICE_INFO:
			BTIF_TRACE_DEBUG(" %s####link_key_write_cback write link key status:%d",__FUNCTION__,status);
			break;
		default:
			BTIF_TRACE_DEBUG(" %s####link_key_write_cback error	status:%d",__FUNCTION__,status);
			break;
...
void btif_dm_save_ble_bonding_keys(void)
{
	...
	tBTM_HOGP_LINK_KEY p ;
    tBTM_SEC_DEV_REC    *p_rec = btm_find_dev (bd_addr.address);
	//if(p_rec && p_rec->dev_class[1] == BTM_COD_MAJOR_PERIPHERAL)
	if(p_rec && hid_known_hogp_device(bd_addr.address))
	{
		for(unsigned int i = 0;i < sizeof(BD_ADDR);i++)
		{
			p.bd_addr[i] = bd_addr.address[5-i];
		}

		p.addr_type =  p_rec->ble.ble_addr_type;
		memcpy(p.ltk,p_rec->ble.keys.ltk,sizeof(p.ltk));
		memcpy(p.rand,p_rec->ble.keys.rand,sizeof(p.rand));
		memcpy(p.irk,p_rec->ble.keys.irk,sizeof(p.irk));
		p.ediv = p_rec->ble.keys.ediv;

        BTIF_TRACE_DEBUG(" %s HOGP add link key",__FUNCTION__);
		BTA_DmNPLSpecificCommand(HCI_VSC_MAIN_NPL,HCI_VSC_HOGP_ADD_DEVICE_INFO,sizeof(tBTM_HOGP_LINK_KEY), (UINT8*)&p, link_key_write_cback);
	}
	else
	{
		BTIF_TRACE_DEBUG(" %s can not find p_rec or it is not hogp device",__FUNCTION__);
	}
