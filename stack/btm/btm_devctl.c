...
/*******************************************************************************
**
** Function         BTM_WriteLocalIRK
**
** Description      Send the local IRK to the controller.
**
** Returns
**      BTM_SUCCESS         Command sent. Does not expect command complete
**                              event. (command cmpl callback param is NULL)
...
**
*******************************************************************************/
tBTM_STATUS BTM_WriteLocalIRK(UINT8 *irk)
{
    UINT8 buf[128];
    UINT8 *p = buf;
    memset(buf,0x0,sizeof(buf));
    UINT8_TO_STREAM(p,HCI_VSC_WRITE_LOCAL_IRK);
    ARRAY_TO_STREAM(p,irk,16);
    UINT8_TO_STREAM(p,BLE_LOCAL_PRIVACY_ENABLED);
    BTM_TRACE_ERROR ("%s:public address is %d",__func__,BLE_LOCAL_PRIVACY_ENABLED);
    return BTM_VendorSpecificCommand(HCI_VSC_MAIN_NPL, 18 ,buf,NULL);
}
...
extern  void save_bt_firmware_version_cback(UINT8 evt_len, UINT8 *p);
/*******************************************************************************
**
...
void btm_vendor_specific_evt (UINT8 *p, UINT8 evt_len)
{
	...
	save_bt_firmware_version_cback(evt_len,p);
}
